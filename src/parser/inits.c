...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/09 14:59:00 by dreule            #+#    #+#             */
/*   Updated: 2025/09/09 14:59:01 by dreule           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	search_for_objects(t_scene *scene, char *parsed_line)
{
	...
	if (!ft_strncmp(parsed_line, "cy", 2))
	{
		...
		normalize_rgb_triplet(&new_node->data->cylinder->material.rgb);
		ftm_tup_norm(&new_node->data->cylinder->axis,
			new_node->data->cylinder->axis);
	}
	search_for_bonus_objects(scene, parsed_line, new_node);
}
